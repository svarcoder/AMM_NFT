...
export function optionalCommaNumber(
	value?: number | string
): string | undefined {
...
export function optionalCommaAmount(
	value?: number | string
): string | undefined {
...
export const currentStep = (steps: any, errors: any, meta?: any) => {
	...
	for (const step of steps) {
		if (
			errors[step] &&
			(!Array.isArray(errors[step]) ||
				(Array.isArray(errors[step]) && errors[step].length > 0))
		) {
			return false;
		}
	}
...
export function genericSum<T>(
	getter: (value: T) => number
): (items: T[]) => number {
	// prettier-ignore
	return items => items
        .map(getter)
...
export function usdString(value: number, decimalPlaces: number = 4): string {
	...
	const result =
		negativePrefix +
		"$" +
		toCommaNumber(Math.abs(value).toFixed(decimalPlaces));
...
export const getViewAddress = (addresses: any, type: any) => {
	const current = addresses?.filter((add: any) =>
		add.groups.some((s: any) => s.id === type)
	)[0];
...
export function getQueryParamFloat(
	queryParams: any,
	key: string
): number | undefined {
